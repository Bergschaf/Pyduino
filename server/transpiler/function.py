...
class Function:
    ...
    def check_definition(instruction: str, transpiler: 'Transpiler'):
        ...
        for arg in args_str:
            arg_name, type_str = arg.split(' ', 1)
            type = PyduinoType.get_type_from_string(type_str)
            if not type:
                transpiler.data.newError(f"Invalid type for argument {name}: '{type_str}'", transpiler.location.getRangeFromString(type_str))
                tyoe = PyduinoUndefined()

            if not StringUtils.is_identifier(arg_name):
                transpiler.data.newError(f"Invalid function argument: '{arg_name}'", transpiler.location.getRangeFromString(arg_name))

            var = Variable(arg_name, type)
            args.append(var)
            transpiler.scope.add_Variable(var, transpiler.location.position.add_line(1))

        for name_str, value_str in kwargs_str:
            name_str, type_str = name_str.split(' ', 1)
            type = PyduinoType.get_type_from_string(type_str)
            if not type:
                transpiler.data.newError(f"Invalid type for argument {name}: '{type_str}'", transpiler.location.getRangeFromString(type_str))
            ...
            if not value:
                transpiler.data.newError(f"Invalid default value for argument {name}: '{value_str}'",
                                         transpiler.location.getRangeFromString(value_str))
                value = Value("None", PyduinoUndefined())

            if not type.is_type(value.type):
                transpiler.data.newError(f"Invalid default value for argument {name}: '{value_str}' is not of type {type}",
                                         transpiler.location.getRangeFromString(value_str))
        ...
        transpiler.data.code_done.append(
            f"{return_type} {name}({', '.join([f'{arg.type} {arg.name}' for arg in args] + [f'{kwarg[0].name} {kwarg[0].type} = {kwarg[1].name}' for kwarg in kwargs])}) {{")
